							...
							<div class="col-md-12">
								<div class="d-flex align-items-center flex-wrap bg-light w-100 rounded p-3 mb-4">
									<div class="d-flex align-items-center justify-content-center avatar avatar-xxl rounded-circle border border-dashed me-2 flex-shrink-0">
										<img src="{{ asset('public/upload/personImg/' .$person->image) }}" alt="Profile Image" class="rounded-circle">
									</div>
    ...
    <select class="form-control select2" name="district">
      ...
      @foreach($districts as $district)
        <option value="{{ $district->id }}"
          {{ (old('district', $person->district) == $district->id) ? 'selected' : '' }}>
          {{ $district->name }}
        </option>
      @endforeach
    </select>
    <span class="text-danger">@error('district') {{ $message }} @enderror</span>
							...
							<div class="col-sm-12">
								<div class="form-group">
									...
									<textarea class="form-control" name="address" placeholder="Enter full address here...">{{ old('address', $person->address) }}</textarea>
