...
<body>
            ...
            <div class="col-lg-10">
                <div class="card">
                    <div class="card-header px-4 py-3">
                        <h4 class="mb-0"><i class="fas fa-fingerprint me-2"></i>Register Biometrics (10 Finger Capture)</h4>
                    </div>
                    <div class="card-body p-4">
                        <form action="#" method="POST">
                            ...
                            <div class="biometric-section">
                                ...
                                <!-- Finger Selection -->
                                <div class="mb-4">
                                    <label class="form-label">Select Finger to Capture:</label>
                                    <div class="finger-selection">
                                        <!-- Left Hand -->
                                        <div class="finger-btn" data-finger="leftThumb">Left Thumb</div>
                                        <div class="finger-btn" data-finger="leftIndex">Left Index</div>
                                        <div class="finger-btn" data-finger="leftMiddle">Left Middle</div>
                                        <div class="finger-btn" data-finger="leftRing">Left Ring</div>
                                        <div class="finger-btn" data-finger="leftLittle">Left Little</div>
                                        
                                        <!-- Right Hand -->
                                        <div class="finger-btn" data-finger="rightThumb">Right Thumb</div>
                                        <div class="finger-btn" data-finger="rightIndex">Right Index</div>
                                        <div class="finger-btn" data-finger="rightMiddle">Right Middle</div>
                                        <div class="finger-btn" data-finger="rightRing">Right Ring</div>
                                        <div class="finger-btn" data-finger="rightLittle">Right Little</div>
                                    </div>
                                    
                                    <div class="batch-capture-btns">
                                        <button type="button" id="captureAllBtn" class="btn btn-outline-primary btn-sm">
                                            <i class="fas fa-bolt me-1"></i>Capture All
                                        </button>
                                ...
                                <div class="capture-summary">
                                    <h6 class="mb-3">Capture Progress</h6>
                            ...
                            <div class="d-flex gap-2 mt-4">
                                <button type="submit" id="submitBtn" class="btn btn-success px-4" disabled>
                                    <i class="fas fa-save me-2"></i>Save All Biometrics
                                </button>
                ...
                <div class="mt-4 p-3 bg-light rounded">
                    ...
                    <ul class="mb-0 small">
                        <li>Make sure the applicant's fingers are clean and dry before capture</li>
                        <li>Press the finger firmly against the scanner without moving</li>
                                        <li>Capture fingers in order from thumb to little finger for both hands</li>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialize capture summary
            function initializeSummary() {
                captureSummary.innerHTML = '';
                allFingers.forEach(finger => {
                    const summaryItem = document.createElement('div');
                    summaryItem.className = 'summary-item pending';
                    summaryItem.id = `summary-${finger}`;
                    summaryItem.innerHTML = `
                        <span class="finger-status status-pending">?</span>
                        <span class="finger-name">${formatFingerName(finger)}</span>
                    `;
                    captureSummary.appendChild(summaryItem);
                });
            }
            
            // Format finger name for display
            function formatFingerName(finger) {
                return finger.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase());
            }
            
            // Update finger button state
            function updateFingerButton(finger, status) {
                const button = document.querySelector(`.finger-btn[data-finger="${finger}"]`);
                button.classList.remove('active', 'captured', 'failed');
                
                if (status === 'captured') {
                    button.classList.add('captured');
                } else if (status === 'failed') {
                    button.classList.add('failed');
                } else if (finger === currentFinger) {
                    button.classList.add('active');
                }
            }
            
            // Update summary item state
            function updateSummaryItem(finger, status) {
                const summaryItem = document.getElementById(`summary-${finger}`);
                const statusIcon = summaryItem.querySelector('.finger-status');
                
                summaryItem.className = 'summary-item ' + (status === 'captured' ? 'captured' : 'pending');
                statusIcon.className = 'finger-status ' + (
                    status === 'captured' ? 'status-captured' : 
                    status === 'failed' ? 'status-failed' : 'status-pending'
                );
                
                statusIcon.textContent = status === 'captured' ? '✓' : 
                                       status === 'failed' ? '✗' : '?';
            }
            
            // Update submit button state based on form completeness
            function updateSubmitButton() {
                const allCaptured = Object.keys(capturedFingers).length >= 10;
                if (applicantSelect.value && allCaptured) {
            ...
            clearAllBtn.addEventListener('click', function() {
                ...
                // Reset UI
                fingerButtons.forEach(btn => {
                    btn.classList.remove('active', 'captured', 'failed');
                });
            ...
            async function tryCapture(url) {
                ...
                return {
                    ...
                    Score: Math.floor(Math.random() * 20) + 60 // Random score between 60-80
                };
            }
            ...
            async function simulateCapture() {
                if (!currentFinger) {
                    alert("Please select a finger to capture first.");
                    return;
                }
                ...
                statusText.textContent = `Capturing ${formatFingerName(currentFinger)}...`;
                statusText.className = 'text-warning';
                scanResult.textContent = `Attempting to capture ${formatFingerName(currentFinger)}...`;
                ...
                try {
                    ...
                    if (cloudFirst) {
                        try {
                            ...
                            scanResult.innerHTML = `<span class='text-success'>✓ ${formatFingerName(currentFinger)} captured via Cloud API</span>\n` + 
                                                  `Quality: ${data.Quality}, Score: ${data.Score}\n` +
                                                  JSON.stringify(data, null, 2);
                        } catch (cloudError) {
                            console.warn("Cloud API failed, trying local service...");
                            try {
                                data = await tryCapture("https://localhost:8000/SGIFPCapture");
                                scanResult.innerHTML = `<span class='text-warning'>⚠ ${formatFingerName(currentFinger)} captured via Local API (Cloud failed)</span>\n` + 
                                                      `Quality: ${data.Quality}, Score: ${data.Score}\n` +
                                                      JSON.stringify(data, null, 2);
                            } catch (localError) {
                        ...
                        try {
                            data = await tryCapture("https://localhost:8000/SGIFPCapture");
                            scanResult.innerHTML = `<span class='text-success'>✓ ${formatFingerName(currentFinger)} captured via Local API</span>\n` + 
                                                  `Quality: ${data.Quality}, Score: ${data.Score}\n` +
                                                  JSON.stringify(data, null, 2);
                        } catch (localError) {
                            ...
                            try {
                                data = await tryCapture("https://webapi.secugen.com/SGIFPCapture");
                                scanResult.innerHTML = `<span class='text-warning'>⚠ ${formatFingerName(currentFinger)} captured via Cloud API (Local failed)</span>\n` + 
                                                      `Quality: ${data.Quality}, Score: ${data.Score}\n` +
                                                      JSON.stringify(data, null, 2);
                            } catch (cloudError) {
                    ...
                    if (data.TemplateBase64) {
                        ...
                        statusText.textContent = `${formatFingerName(currentFinger)} captured successfully`;
                        statusText.className = 'text-success';
                        
                        // Show captured fingerprint image
                        capturedFingerprint.style.display = 'block';
                        
                        // Update finger button and summary
                        updateFingerButton(currentFinger, 'captured');
                        updateSummaryItem(currentFinger, 'captured');
                        
                        // Enable submit button if all fingers are captured and applicant is selected
                        updateSubmitButton();
                    } else {
                    ...
                    scanResult.textContent = `Error capturing ${formatFingerName(currentFinger)}: ` + error.message;
                    statusIndicator.className = 'status-indicator status-error';
                    statusText.textContent = `${formatFingerName(currentFinger)} capture failed`;
                    statusText.className = 'text-danger';
                    
                    // Update finger button and summary
                    updateFingerButton(currentFinger, 'failed');
                    updateSummaryItem(currentFinger, 'failed');
                    ...
                    if (error.message.includes('Failed to connect')) {
                        scanResult.innerHTML = `<span class='text-danger'>Connection error:</span> Unable to reach the fingerprint capture service for ${formatFingerName(currentFinger)}. ` +
            ...
            // Set up scan button handler
            scanBtn.addEventListener('click', simulateCapture);
